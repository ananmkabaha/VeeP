...
class Network:
    if os.name == 'nt': # Running in Windows
        ...
        _lib = ctypes.cdll.LoadLibrary(os.path.join(os.path.dirname(os.path.abspath(__file__)),"./libgpupoly.so.0.13"))
    ...
    def __init__(self, input_size,gpu_to_use):
    ...
    def test(self, down, up, label, soundness=True):
        ...
        if up.dtype == np.float64:
            return self._lib.test_d(self._nn, down, up, label, soundness),time.time()-start_
        return self._lib.test_s(self._nn, down, up, label, soundness),time.time()-start_
